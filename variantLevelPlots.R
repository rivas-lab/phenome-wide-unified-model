...
server <- function(input, output, session) {
  ...
  paste("Unified Model p-value:", formatC(gene_results_filtered()$p_model, format = "e", digits = 3))
})
...
output$constraintPlot <- renderPlotly({
    ...
    p_constraint <- paste("(p-value:", formatC(gene_results_filtered()$p_log_constraint, format = "e", digits = 3), ")")
...
output$pathogenicityPlot <- renderPlotly({
    ...
    constant <- gene_results_filtered()$coef_constant[1]
    p_pathogenicity <- paste("(p-value:", formatC(gene_results_filtered()$p_log_pathogenicity, format = "e", digits = 3), ")")
    ...
    p_lof <- paste("(p-value:", formatC(gene_results_filtered()$p_pLoF_indicator, format = "e", digits = 3), ")")
...
output$missensePlot <- renderPlotly({
    ...
    p_missense <- paste("(p-value:", formatC(gene_results_filtered()$p_missense_indicator, format = "e", digits = 3), ")")
